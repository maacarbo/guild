...
Guild is an open-source autonomous-SDLC governance layer driving a self-hosted Multica execution substrate (stages, approval gates, handoff contracts, budget enforcement). Read these before writing code; they are normative. Architecture rationale lives in `docs/ARCHITECTURE.md` (decision records, D1 onward); this file operationalizes it.

| Doc | Contents |
|---|---|
| `docs/PRODUCT.md` | Vision, flows, MVP cut |
| `docs/ARCHITECTURE.md` | Decision records (D1 onward), engagement lifecycle, deployment topology |
| `docs/AGENT-MANUAL.md` | Agent-facing how-tos with CI-checked coverage markers |
...
| `docs/VALIDATION-*.md`, `docs/research/` | Historical evidence records — append dated addenda only; never revise existing text |
...
- **License guardrail (D8):** never host Multica for third parties — even free of charge (Multica License condition 1a, accepted 2026-08-17 with the v0.4.26 bump) — embed it in anything sold, or rebrand its UI; keep the "built on Multica" attribution links in README/PRODUCT (condition 1c). Pin the Multica version; review its LICENSE diff on every upgrade before bumping.
