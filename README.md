...
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | Decision records (D1 onward, with honest superseded/retained statuses), engagement lifecycle, deployment topology |
| [docs/AGENT-MANUAL.md](docs/AGENT-MANUAL.md) | The agent-facing manual: how-tos for driving Guild end to end, each backed by a named test or check |
...
*(as of 2026-08-04)* **M1, M2a, and M2b functionally complete — the full application runs end to end on the live stack.** M1 (accepted 2026-07-31) proved the substrate: the Tier 1 compose stack with the doctor diagnostic, the pinned daemon image (OpenCode default; Claude Code was in scope then and removed by the 2026-08-04 D9 amendment), the `ExecutionSubstrate` port with its Multica adapter and conformance suite (the standing pin-bump gate), and the first SHA-pinned contract validation through the sandboxed `docker run` driver. M2a (accepted 2026-08-03) ran one real engagement through the whole governed lifecycle including a genuine bounce and a conductor-kill recovery. M2b (accepted 2026-08-04, `pnpm smoke:m2b` green twice) is the whole product loop: an operator idea posted as a board ticket became a deterministic five-stage plan, was amended and re-gated live, and was delivered by the fixed four-role team through five sequential operator-approved gates — a repo whose tests pass in a fresh clone, at a measured 77–111¢ against a $3.00 plan — while an induced overspend halted the pipeline cleanly (work cancelled, dispatch locked, explanation on the ticket) and the decision trail audited to zero un-contracted advances. The conductor ships as a compose service with `guild init` / `guild demo` / `guild kill` verbs, and the fresh-machine bar ran for real: `down -v` → quickstart verbatim → first accepted stage in 4.5 minutes, full delivery in 17.5 (bar: ≤30). **`v0.2.0` is the current release** — M3 team evolution, the feature-complete product: API-backed dynamic hiring (D16), archive-only retirement, role memory across engagements, the template catalog (`standard`/`quick-fix`), and run-pinned project rules — accepted live 2026-08-12 (a tester hired because the plan demanded it, contracted work delivered through gates, retired on completion; a second run's tester received the first run's role memory; hire/retire rows queryable in `decisions`). `v0.1.1` was the prior security patch (#17, advisory `GHSA-7pg8-mmpv-r6pc`), a **breaking** upgrade for `v0.1.0` installs (see `deploy/compose/README.md`). Kubernetes stays an optional last milestone (M4). Every design claim above traces to cited evidence in `docs/`.
