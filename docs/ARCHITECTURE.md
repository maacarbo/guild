...
- **Dev mode carries the cheap half of the hardening from day one** (2026-07-30, external review): dedicated namespaces, deny-by-default NetworkPolicies (per the `toFQDNs` design above), non-privileged service accounts with `automountServiceAccountToken: false`, PSA `restricted` labels on the dev namespaces, and the narrower `mdt_` daemon token where its scope suffices.
