...
- **Bounce continuity is best-effort, not guaranteed.** Session resume requires same agent + issue + workdir + non-poisoned session; a pod restart breaks it. Bounce comments are therefore **self-contained** (brief + verdict + failing criteria — enough to proceed context-fresh), and bounce-cost accounting assumes fresh context as the floor.
...
- Budget: the watchdog meters gateway spend per engagement; soft cap → warn, hard cap → cancel via substrate + stop dispatching. **Attribution mechanism (2026-07-30, external review — a container-wide base-URL env carries no engagement identity):** primary design is a **per-engagement LiteLLM virtual key minted at dispatch**; fallbacks are per-agent keys (role-level attribution) or header injection via a thin proxy. **M1 must prove one mechanism end to end before the watchdog is built** — until then the budget feature is unproven. **Insurance before the watchdog exists (restores the July-29 validation's M1-era cap):** every engagement key is minted with LiteLLM's native `max_budget = budgetCents`, so the gateway itself stops serving at the cap even with zero watchdog code running; M1a proves the key actually stops at cap and how the resulting 429 classifies in Multica. The M3 watchdog adds soft-cap warnings, project-level aggregation (`ProjectBudget`), and key cleanup. Semantics: money is integer cents; caps trigger at `spent >= cap`; hard cap cancels in-flight work and locks dispatch; the gateway is the source of truth when telemetry lags (Multica `task_usage` is optional reconciliation only).

## Kubernetes topology (the M3 form — M1–M2 run the same components as Docker Compose services; simple-to-complex, operator decision 2026-07-30)

**Portability note (2026-07-30):** the component/namespace/trust topology below is normative for any Kubernetes deployment (Tier 2 in `deploy/README.md`; a Docker-Compose Tier 1 exists with the same components and validator sandboxes as `docker run` containers). Bullets naming concrete mechanisms — Cilium `toFQDNs`, gVisor via Talos system extensions, NFS classes — are the **Tier 3 reference implementation** of generic requirements (FQDN egress control, runtime sandboxing, safe storage); other environments provide equivalents where their infrastructure supports them, or accept the documented residual risk.
...
- **Daemon Deployment is Guild's contribution**: custom image (multica binary + agent CLIs + git — binary and CLI versions pinned as build args, CLI autoupdaters disabled; chart + image declared a lockstep pair), headless `multica login --token` from a Secret. Egress control is a **CiliumNetworkPolicy with `toFQDNs`** for git hosts (their IP ranges churn — a CIDR allowlist is forbidden as unmaintainable) plus the **mandatory DNS-proxy rule** `toFQDNs` requires; git over HTTPS only; Multica backend and gateway by cluster identity.
...
- **Compose-era (M1–M2) hardening floor**: dedicated compose network, non-root containers, no unnecessary host mounts — with the real blast-radius bounds being the explicit-approval default and gateway `max_budget` caps. **At the M3 lift**: dedicated namespaces, the deny-by-default `toFQDNs` NetworkPolicies above, non-privileged service accounts with `automountServiceAccountToken: false`, PSA `restricted` labels, and the narrower `mdt_` daemon token where its scope suffices.
- **gVisor on Talos is a node-image project, not a toggle** (Anthropic review 2026-07-30): it requires a Talos system extension in the machine schematic and a node upgrade/reboot. Plan: extend the schematic on **one labeled worker**, install the RuntimeClass, pin sandboxed workloads there via nodeSelector, smoke-test, and benchmark I/O on representative builds; M3's "mandatory" means extending the schematic cluster-wide via the promotion runbook — it is not a Flux-deliverable.
