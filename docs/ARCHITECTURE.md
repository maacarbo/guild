...
| `packages/substrate-conformance` | The reusable `ExecutionSubstrate` port contract suite (against the live stack; every suite test is named by a scenario in its feature file) — mandatory-green on every Multica pin bump and daemon image rebuild. |
...
- **The validator is itself least-trusted.** Contract checks execute agent-authored commands against agent-authored code — hostile input. Validation therefore runs as an **ephemeral sandbox** — zero Guild credentials, no gateway master key, no Guild PG access, registry-only egress. *(Amendment 2026-08-16, conformance audit docs-6: the sandbox image was originally described as "the daemon image"; what ships on Tier 1 is a pinned minimal Node image — `GUILD_VALIDATOR_IMAGE`, default `node:22-alpine` — smaller than the daemon image and equally credential-free, selectable per install; the K8s driver would use a Job from the same image family. The trust argument rests on credential-freedom and sandbox ephemerality, not on image identity.)* The conductor's validator runner has two drivers with the same contract: a K8s Job (Kubernetes deployments) and a `docker run` container (Tier 1 compose). The conductor reads back exit codes and captured evidence and **writes the verdict itself** — the validator can fail the work; it can never sign for Guild.
...
| Option | Pros | Cons |
|---|---|---|
| **One workspace per Guild project** ✔ | Hard isolation by construction — board, agents, and repo wiring are workspace-scoped (workspace-level attachment is the only repo mechanism that exists, P18); one board wall per project (composes with D11); workspace creation is API-automatable (`POST /api/workspaces`, `name`+`slug` required); **one daemon serves every workspace its user is a member of** (P17: runtime rows project the moment membership lands, no restart, and task service there is live-proven; under D15's non-owner daemon identity, joining a new workspace takes the invite/accept handshake — both calls API-automatable, matrix addendum 2026-08-13) | More substrate objects per project (workspace, agents, runtime-row views); per-workspace provisioning code |
...
- **CLI scope:** `guild init`, `guild doctor`, and the emergency kill-switch. Nothing else. **Amendment (2026-08-16, conformance audit docs-1):** a fourth verb, `guild demo`, shipped with the M2 quickstart as a fixture/acceptance verb (it posts the known-good demo idea; the M2 ship-gate in ROADMAP runs it) and was never recorded here — a silent scope change, recorded now.

  | Option | Pros | Cons |
  |---|---|---|
  | **Record `demo` as a fixture verb, governance surface stays board-only** ✔ | It exists, it is load-bearing for the documented quickstart and the M2 ship-gate, and it adds no governance authority (it only posts a ticket the operator could post by hand) | The "nothing else" phrasing needed this amendment |
  | Delete the verb to restore the original scope | The record stays literally true | Breaks the documented quickstart and the M2 ship-gate for a wording victory |
...
**Amendment (2026-08-13, #28 — stage identity splits from stage kind, operator-ratified):** the documented enterprise shape needs two same-kind stages, and stage identity embedded the kind (`stg:<ideaId>:<kind>` — stage ids, gate markers, and handoff paths would all collide).

| Option | Pros | Cons |
|---|---|---|
| **Template entries `{slug, kind}`: slug = identity, kind = behavior class** ✔ | `@guild/shared`'s five-kind union stays stable; stage ids, gate markers, handoff paths, and the role-memory phrasing key on the SLUG (`stg:<ideaId>:<slug>`), while floor checks, missions, deliverables, and default roles stay kind-derived — with per-entry `mission`/`role` overrides as catalog data; the classic templates keep slug == kind, so every existing id, marker, path, and memory line is byte-identical: no migration; determinism and zero discretion hold (the catalog is fixed data behind the `template:` directive) | A second identity word ("slug") joins the ubiquitous language; the conductor resolves slug → entry via the active template instead of casting the id suffix |
| Expand the `StageKind` union per flavor | Compiler-enforced ripple | The published language churns per template idea; every flavor needs its own floor-check content even when behaviorally identical |
| Two stages share a kind, types untouched | Zero type work | Dead on identity — stageId/gate-marker/handoff-path collisions |

The `enterprise` template ships as data with this amendment: business-analysis → technical-analysis → architecture+security (mission-extended) → implementation → test → delivery (10/10/15/35/20/10). The first template stage is operator-authored regardless of its kind. Per-entry **floor-check overrides** joined the catalog data 2026-08-16 (conformance audit, bdd-0): same-kind stages must ship *discriminating* floors — with kind-derived floors alone, technical-analysis inherited business-analysis's identical `docs/SPEC.md` bar, which acceptance fast-forward had already satisfied, so its contract could not fail; the enterprise entries now require `## Technical constraints` (technical-analysis) and `## Security` (architecture-security) as artifact checks, deterministic and operator-visible at the gate like every floor. Since 2026-08-16 (audit bdd-4, #58) the floor gherkin derives its Then-steps from the floor checks themselves — concrete criteria per stage, no self-report step — so the contract's human-readable half states exactly what the validator executes (D6: a contract neither restates the validation mechanism nor pivots on the role's self-report). A slug absent from the re-resolved template (a mid-run `template:` edit) degrades deterministically to a 0¢ allocation — surfaced by the existing 0¢ gate-body warning. Known limitation (verification finding, tracked on #28): the same mid-run template edit can make the *previous* stage's entry unresolvable, in which case the next gate's `authoredBy` provenance falls back to a kind-guessed role without a warning — narrow (requires editing `template:` between stages of a live run), operator-visible only in the gate body's authorship line.

**Amendment (2026-08-13, #35 — handoff checks must be stage-appropriate):** upstream-authored checks gate the stage they are authored *for*, so the brief's handoff-authoring instruction requires every check to be satisfiable by that stage's **own deliverable** (named per stage kind: architecture = design documents under `docs/`, and so on), never by later stages' artifacts — the v0.2.0 release-gate smokes showed every analyst model authoring architecture checks that execute the not-yet-written implementation, driving the stage into bounce escalation exactly as designed.

| Option | Pros | Cons |
|---|---|---|
| **Instruction names the deliverable + advisory gate-body warning for docs-only stages** ✔ | The operator sees the offending check at the approval they were already making and amends or bounces with guidance; a legitimate unusual check still merges — Guild proposes, the operator disposes | The warning is advisory: an inattentive approval still admits the doomed check |
| Reject out-of-scope upstream checks outright | Mechanical, no operator attention needed | An upstream role gains a silent veto over the next stage's contract; false positives on legitimate cross-artifact checks are unappealable |
| Instruction-only, no warning | Smallest change | Exactly the state the v0.2.0 smokes found failing — models ignored the instruction and nothing surfaced it |

Two mechanics: (1) the instruction names the next stage's deliverable and the escalation consequence; (2) for docs-only stages (analysis, architecture) the gate body renders a warning when an upstream check references a path outside `docs/` — advisory, the check still merges: the operator amends or bounces with guidance before approving. The brief's next-stage computation follows the **active template's** stage list, so a quick-fix tester is never told to author checks for a delivery stage the template does not run.
...
- **Project layer:** one `AGENTS.md` rules file in the project repo, read at a SHA **pinned once at idea adoption** (the target branch's head, stored on the plan run) and folded into every brief of that run with provenance — D6's "briefs carry everything" is unchanged. *(Precision, operator decision 2026-08-11: the original "at the validated SHA" phrasing was per-stage and ambiguous for stage 1, which has no preceding validated SHA; one immutable rules version per run matches how `planVersion` pins a stage's inputs and keeps same-run briefs reproducible. Rules changes take effect on the NEXT adopted idea.)*
...
Mechanics shipped with the amendment: `stageComplete` skips advisory riders (≥ 1 non-advisory acceptance still required — an all-advisory stage never completes, the same deliberate fail-safe wedge an empty stage always had: a stage that can produce no acceptable work stalls visibly rather than advancing silently); the advance loop closes a completed stage's riders idempotently; `judgeReported` ignores advisory reports — no contract, no verdict, no bounce (observations are ticket comments). Since 2026-08-16 (audit ddd-5, #54) the gate body renders a rider as what it is — "advisory rider — observe-and-flag; never validated, cancelled at stage end", cost visible, its placeholder contract suppressed — so approval covers the rider's actual deal, not criteria the conductor will never judge. The planner emits no advisory engagements yet — the opt-in monitor role template arrives with charter-driven role requests and must emit ≥ 1 non-advisory engagement per stage; the semantics no longer block it. Still scoped to land WITH that producer (#54 stays open): dropping the placeholder `EngagementBrief.contract` a rider is forced to carry, and the report-path plan-read skip.
...
- **The contract validator is a second least-trusted workload**: an ephemeral sandbox from the pinned minimal validator image (`GUILD_VALIDATOR_IMAGE`, default `node:22-alpine` — see the D6 amendment 2026-08-16), zero Guild credentials, registry-only egress (see D6) — the conductor reads its exit codes/evidence and writes the verdict itself.
...
### D16 — Hiring is API-backed; retirement is archive, one-way (added 2026-08-11, M3)

**Decision (operator-ratified 2026-08-11):** Guild hires by creating Multica agents through the management REST surface at dispatch time — the plan demanding a role IS the hiring signal — and retires by archiving, never restoring. The pre-declared idle-pool fallback is **dropped**: it hedged an unverified surface that is now verified at every tier.

**Evidence (live, guild-dev stack, Multica v0.4.15, conductor token):** `POST /api/agents` → 201 (requires an existing `runtime_id`: 400 without — agents are never created unborn); `PUT /api/agents/{id}` (rename/model) → 200; `PUT /api/agents/{id}/env` → 200; retire = `POST /api/agents/{id}/archive` → 200 (`DELETE` is 405; re-archive is 409 "agent is already archived"; archived names stay reserved, so hire names are unique per (role, run)); `/restore` exists and is **forbidden by this decision**. The decisive probe: an agent created at runtime, assigned a fresh issue, reached `task_run running` with **no daemon restart** — the dispatchability claim every earlier probe (incl. frozen P15) left at CRUD level.

**Amendment (2026-08-17, v0.4.26 bump — PR #47):** upstream MUL-6126 makes private-runtime agent **create/move owner-only** (the conductor's workspace-admin rights no longer suffice; archive and env updates remain admin-allowed — re-probed live). Hiring and rebinding now ride the **daemon (runtime-owner) credential**: the anti-corruption adapter takes an `agentLifecycleToken` used for `createAgent` only, the conductor's composition root reads `MULTICA_DAEMON_TOKEN`, and `guild init` provisions the starter team under the daemon identity. Because daemon-created agents are additionally **private to their owner for work assignment** ("cannot assign to private agent"), every hire is created `permission_mode: public_to` with an invocation allow-list of exactly its dispatcher — `selfMemberId` (the conductor) for runtime hires, the conductor member for `guild init`'s starter team; least privilege, probed live 2026-08-17. Attribution and the D15 partition are unchanged — hires are recorded in Guild's own decision trail, the daemon stays off the operator allowlist (A5d), and every board/governance action keeps the conductor's token. Residual (upstream's model, stated plainly): the agent-reachable daemon token can create/move agents on its own runtime; such an agent has no engagement key, no budget, and is never dispatched by Guild — board noise, not a governance breach.

| Alternative | Why not |
|---|---|
| Pre-registered idle-pool fallback (2026-07-30 hedge) | Hedged only the unverified creation surface, which is now live-verified end-to-end incl. dispatchability; a pool adds standing registry state and a second staffing mechanism for zero residual risk |
| Retire = `/restore`-on-rehire (reuse agent identity) | Reopens two invariants at once: one-agent-one-open-engagement, and stale `custom_env`/attribution from the prior engagement; role continuity is the role-memory artifact's job, not the agent row's |
| Retire = hard delete | The API refuses (405); archive keeps history readable — the read path is archive-tolerant by construction |
...
Custody model and its honest boundary: Guild never holds a credential value — the conductor config names the variable (`GUILD_GIT_CRED_NAME`), the substrate folds the name into every engagement bind, and the daemon's helper resolves it at git time, falling back to the ambient bootstrap store. The helper (`docker/daemon/guild-git-cred.sh`, a tracked artifact with an executable spec in `tools/checks/` — hardened 2026-08-16, conformance audit) expands **only the `GUILD_GIT_TOKEN_*` namespace** with a clean `[A-Z0-9_]` shape: the name is agent-reachable input, and a merely well-shaped name is not a safe name — an agent naming `PATH` or `MULTICA_DAEMON_TOKEN` must never see it echoed as a git password. It is also **get-only** (git's `store`/`erase` succeed as no-ops: the ambient store is entrypoint-seeded, never runtime-written) and rejects token values carrying newlines (credential-protocol injection). Since 2026-08-16 (audit clean-1, #56) the helper also reads git's request and honors **host scoping**: `GUILD_GIT_CRED_HOST`, set beside the token name, pins the token to exactly that request host — a GitLab-minted token is never offered to github.com; unset means unscoped (single-host deployments unchanged). **Within one daemon container, any agent can still read any variable the container holds via its own shell** — per-project tokens in a shared daemon bound blast radius at the host/scope level (a token reaches only its repos), not between projects sharing that daemon; a project needing intra-daemon isolation runs its own daemon service (the declared daemon-per-project compose pattern). The ambient PAT shrinks to bootstrap as projects opt in. Evidence: probe P6 (2026-08-13, scratch branch `probe/p6`). **Revisit if:** GitHub App integration lands (automatable GitHub minting + short-lived tokens), or multi-project deployments make daemon-per-project the default topology.
