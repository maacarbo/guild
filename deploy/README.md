...
Secrets are created directly by the operator — gitignored `.env` files (`chmod 600`) on Tier 1, the primary path; `kubectl create secret generic --from-env-file=...` on Tier 2; this **is** the supported path. Only **two values are user-typed external credentials** (`ANTHROPIC_API_KEY` and the scoped git PAT); the rest is generated by the bootstrap script, or minted after Multica boots (`mul_`/`mdt_` — a documented ordering step, not a surprise). Names are normative so any later secrets operator (e.g. External Secrets Operator) is a source swap, not a rename:
