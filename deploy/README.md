...
Guild's daemon executes LLM-generated code inside standard (runc) containers on your machine. **What this tier provides:** container isolation with non-root users, no host mounts into agent-reachable containers, a contained network posture (no database publishes a host port — the M1–M2a dev-era loopback publish of guild-postgres closed when the conductor shipped as a compose service at M2b; the host-side dev harness re-adds it only via the explicit `docker-compose.dev.yml` override, never in the base file — and the validator sandbox runs with no network at all, `--network none`), dropped capabilities + `no-new-privileges`, and memory/pids limits on daemon and validator containers. **What this tier does NOT provide:** kernel-level sandboxing (a container escape is host compromise), egress control (a prompt-injected agent can send anything it can read — including its git token and Multica daemon token — to any host on the internet, and can push to any repo its git token reaches; note the daemon token is a **non-governance** identity since D15/#17 A5d, so exfiltrating it grants agent-level board access, not the power to forge an operator approval), enforced registry-only egress for the validator sandbox (a documented D6 deviation), and full in-stack segmentation beyond the data tier (since 2026-08-16, #57, the databases live ONLY on the `internal: true` `guild-data` network — the daemon has no route to any Postgres — but the daemon still shares `guild-dev` with the backend, frontend, gateway, and conductor, so those service surfaces remain reachable from LLM-run code; the validator sandbox stays networkless). The conductor's Docker API route is a scoped socket proxy (never a raw socket mount), but the proxy's granularity is method-level, not payload-level: a compromised conductor could still create privileged containers through it — the conductor is trusted Guild code, and this residual reach is part of the accepted Tier 1 posture. **Blast radius is bounded economically and procedurally, not by the network:** provider API keys never leave the gateway; every engagement key is minted with a hard `max_budget` the gateway itself enforces; plans require explicit approval before any spend; merges are Guild-mediated and fast-forward-only to the validated SHA. Run Guild on a machine, and with a git credential, you are prepared to lose: use a **fine-grained PAT scoped to the product repo(s) only** — ideally under a dedicated account/org. Plaintext `.env` secrets on a single host are part of this accepted posture. Kernel sandboxing (gVisor/Kata) and FQDN egress allowlists exist only in the Kubernetes tier; if you need them, that tier is the migration path.
...
| `guild-daemon` | daemon | `MULTICA_DAEMON_TOKEN` (`mul_…` PAT for the **`daemon@guild.local`** member — a dedicated **non-governance** identity minted by `guild init`, NOT the operator's own account; D15/#17 A5d. The narrower workspace-scoped `mdt_` token was not exercised in M1a and is a follow-up), git credentials (fine-grained HTTPS PAT scoped to the product repo(s) only), agent-CLI env |
| `litellm-app` | gateway | `ANTHROPIC_API_KEY` (+ optional OpenRouter), master key, DB creds |
| `guild-conductor` | conductor | Multica PAT (`mul_…`), LiteLLM admin key (key minting), Guild PG creds, `GUILD_OPERATOR_MEMBER_IDS` (the D15 operator allowlist) + `GUILD_DAEMON_MEMBER_ID` (asserted off the allowlist at startup) — both printed by `guild init` |
