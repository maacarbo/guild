...
3. **Point the conductor at the name**: set `GUILD_GIT_CRED_NAME=GUILD_GIT_TOKEN_MYPROJ`
   ...
   name to pin the token to one request host (#56): any other host falls
   through to the ambient store, so a multi-host daemon never offers one
   host's token to another. For GitLab set `GUILD_GIT_CRED_USERNAME` on the
